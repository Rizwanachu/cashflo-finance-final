...
export function exportAnalyticsToCsv(
  ...
  const categoryHeader = [\"Category\", \"Income\", \"Expense\", \"Net\"];
  const categoryRows = Object.entries(categoryTotals).map(([catId, totals]) => {
  ...
  const monthHeader = [\"Month\", \"Income\", \"Expense\", \"Net\"];
  const monthRows = Object.entries(monthlyTotals).map(([month, totals]) => [
    month,
    totals.income.toFixed(2),
    totals.expense.toFixed(2),
    (totals.income - totals.expense).toFixed(2)
  ...
  const csvContent = [
    ...
    monthHeader,
    ...monthRows
  ].map(row => row.map(cell => escapeCsvValue(String(cell))).join(\",\")).join(\"\\n\");
...
export function exportAnalyticsToPdf(
  ...
  doc.text(`Generated: ${now.toLocaleString(\"en-GB\")}`, 14, 28);
  ...
  const categoryData = Object.entries(categoryTotals).map(([catId, totals]) => {
    const cat = categories.find(c => c.id === catId);
    return [
      cat?.name ?? catId,
      `${symbol}${totals.income.toFixed(2)}`,
      `${symbol}${totals.expense.toFixed(2)}`,
      `${symbol}${(totals.income - totals.expense).toFixed(2)}`
    ];
  });

  doc.setFontSize(14);
  doc.text(\"Category Breakdown\", 14, 40);
  ...
  const monthlyData = Object.entries(monthlyTotals).map(([month, totals]) => [
    month,
    `${symbol}${totals.income.toFixed(2)}`,
    `${symbol}${totals.expense.toFixed(2)}`,
    `${symbol}${(totals.income - totals.expense).toFixed(2)}`
  ...
  doc.save(`analytics-${datePart}.pdf`);
}
