...
const BudgetOverview: React.FC = () => {
  ...
  useEffect(() => {
    setOverallInput(budgets.overall ? budgets.overall.toString() : "");
  }, [budgets.overall]);
    ...
    <div className="rounded-2xl bg-white border border-slate-200 p-4 shadow-sm dark:bg-[var(--bg-tertiary)] dark:border-[var(--border-subtle)]">
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-1">
          {Object.entries(budgets.perCategory).map(([catId, limit]) => {
            const spent = monthlyExpensesByCategory[catId] || 0;
            const percent =
              limit && limit > 0
